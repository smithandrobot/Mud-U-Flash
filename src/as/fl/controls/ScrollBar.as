...
package fl.controls {	
	...
	public class ScrollBar extends UIComponent {
		...
		override protected function configUI():void {
			...
			downArrow.setSize(WIDTH,24);
			downArrow.autoRepeat = true;
			downArrow.focusEnabled = false;
			addChild(downArrow);
			
			upArrow = new BaseButton();
			upArrow.setSize(WIDTH,24);
			upArrow.move(0,0);
		...
		protected function updateThumb():void {
			...
			var minThumbHeight = 51;
			if (track.height <= minThumbHeight || _maxScrollPosition <= _minScrollPosition || (per == 0 || isNaN(per))) {
				thumb.height = minThumbHeight;
				thumb.visible = false;
			} else {
				thumb.height = Math.max(minThumbHeight,_pageSize / per * track.height);
